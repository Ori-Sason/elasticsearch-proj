...
- Avoid absolute language ("always", "never", "essentially never") for a technical claim unless it's true in literally every case. If there's a real edge case — even a narrow or coincidental one — name it explicitly rather than rounding it away.
...
3. **Walkthrough** — one flowing set of sections organized by topic, in the order the session actually built understanding — not split into a theory phase and a doing phase. Each topic gets a heading, and under it: the real explanation (what it is, why it exists, the mechanism behind it, how it compares to things the user already knows — Postgres, MongoDB, AWS, Docker/Kubernetes, etc., sharpening understanding rather than replacing it), followed by a paragraph starting **`**Hands-on:**`** that gives the actual steps/commands/config/output demonstrating it — what was run, what it returned, what that meant, and what was decided (including alternatives considered and why they were or weren't used). Use real code/command blocks, not paraphrases. When a command or query produces output, include a short real output snippet (e.g. trimmed JSON) alongside the prose, not just a description of what it returned — show one representative record/result and truncate repeated ones with `...` rather than pasting everything. Link out to files in the repo (`docker-compose.yaml`, scripts) for their full content instead of duplicating something long, but still explain and quote the parts that matter. If a command/config's exact syntax is uncertain, verify it via a documentation-lookup MCP (e.g. `context7`, if available) before writing it into the note as fact. When a topic's explanation covers an abstract mechanism with real cost/scaling behavior (e.g. why some cost grows with a parameter), include a small, simple worked numeric example — concrete small numbers plugged in — alongside the prose, not just the abstract description.
...
- Don't narrate the session as a back-and-forth recap ("asked to explain X, the answer given was...", "the user then confirmed..."). This also covers narrating the *verification process* itself, even without literal Q&A wording — e.g. "predicted 0 hits before running, correctly" or "confirmed by computing..." are still a story of the exchange, not the fact. Write the underlying fact or mechanism directly as informational content instead (state the reasoning and the result as one fact, not a predict-then-check narrative) — check whether it's already covered elsewhere in the note first, and only add it if it isn't.
