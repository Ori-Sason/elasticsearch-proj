...
**Tasks:**
- [x] Design a log document schema: `timestamp`, `level`, `service`, `message`, `status_code` (add fields if useful, but keep it small)
- [x] Create the index with an explicit mapping (correct field types — `date`, `keyword` vs `text`, `integer` — chosen deliberately, not left to dynamic mapping)
- [x] Write a Python script generating ~5,000 synthetic log lines across a handful of fake services, log levels, and status codes
- [x] Bulk-index them using the Python Elasticsearch client's bulk helper, and verify the doc count matches what you generated
...
- [x] Closing hands-on: run `GET <index>/_analyze` against the `message` field and against a `keyword` field to see the actual tokens each produces, then deliberately try to change a mapped field's type on the existing index and observe the error, to feel the immutability constraint firsthand
...
**Tasks:**
- [x] Design an explicit SQLite `logs` table schema mirroring the session 2 ES mapping (same fields, appropriate SQLite types — consistent with this project's explicit-schema convention)
...
- [x] Write a backfill script that reads the existing documents out of the session 2 ES index and inserts them into the new SQLite table, making SQLite the source of truth for the first time
- [x] Write a separate reindex/sync script that rebuilds the ES index from SQLite (not from the original generator) — this is the "ES is derived from the DB" pattern made concrete, not just asserted
- [x] Extend the session 5 TypeScript API with a write path (new or modified endpoint) that inserts a new log into SQLite first, then indexes it into ES — and verify a round trip: write through the API, confirm the row in SQLite, confirm the doc in ES
- [x] Add the SQLite file path to `.env.example` alongside the existing ES connection config, consistent with keeping config explicit rather than hardcoded
...
- [x] Closing hands-on: deliberately simulate a failed ES write in the dual-write endpoint by killing ES mid-request (e.g. `docker compose stop elasticsearch`) — not by pointing it at a bad/nonexistent index name, since ES auto-creates a missing index on write by default (`action.auto_create_index`), so that write would silently succeed into a new, dynamically-mapped index instead of failing. Confirm the SQLite row still exists and ES doesn't, then run the reindex script and confirm ES catches back up — a hands-on recovery from the exact drift the deep dive just described
